...
rewrite predeqE => p; split.
  move=> plim_p A B [M [Mreal ygtM_A]] /locallyP [e egt0 pe_B].
...
Proof. exact: proper_pinfty_locally. Qed.
...
rewrite propeqE; split.
  move/(@locallyP _ [the pseudoMetricType _ of R^o]).
...
move=> Kp t0ge0; split=> [t tlt0|t tge0].
  by rewrite /shift_sol leNgt tlt0 lexx add0r ltW ?oppr_gt0.
...
move: tge0; rewrite le_eqVlt orbC => /orP [tgt0|/eqP teq0].
  ...
  suff -> /= : (0 <= s + t)%R.
    ...
    by case: e => /=; apply/(eqoP (locally_filter_on (0%R : R))).
...
set u := (X in `|X x|%R).
near: x.
have := (@eqoP _ _ _ _ (locally_filter_on (0%R : R^o)) id u).1.
exact.
Grab Existential Variables. all: end_near. Qed.
...
rewrite [shift_sol _ _ _]/shift_sol lexx add0r => <-; last exact: Kinvar.
